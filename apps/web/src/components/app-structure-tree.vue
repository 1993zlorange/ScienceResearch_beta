<template>
  <nav class="structure-tree" data-testid="app-structure-tree" aria-label="结构树">
    <RouterLink
    ...
    <RouterLink
      class="tree-root-link"
      to="/technical-docs"
      :aria-current="route.path === '/technical-docs' ? 'page' : undefined"
    >技术说明</RouterLink>
    ...
    <div class="tree-node">
      <button
        type="button"
        class="tree-toggle"
        :class="{ active: isContextFlow }"
        :aria-expanded="contextFlowExpanded ? 'true' : 'false'"
        aria-controls="structure-context-flow"
        data-testid="structure-toggle-context-flow"
        @click="toggle('context-flow')"
      >
        <span>课题流程</span>
        <span aria-hidden="true">{{ contextFlowExpanded ? '−' : '+' }}</span>
      </button>
      <div v-if="contextFlowExpanded" id="structure-context-flow" class="tree-children">
        <RouterLink
          class="tree-link"
          to="/contexts"
